...
# Calculates the mean phastCons score for each CRE.

START=$(date +%s)

MUS="/Xnfs/lbmcdb/Semon_team/jcaron/files/bedfiles/mus_links.bed"
PHAST_DIR="/Xnfs/lbmcdb/Semon_team/jcaron/files/mdongScores/PhastCons_53"
OUT="/Xnfs/lbmcdb/Semon_team/jcaron/files/bedfiles/mus_phastcons_v2.bed"
...
echo "[2/4] Indexing PhastCons window files..."
# Build a lookup table of the per-window PhastCons files: chrom, window
# start, window end, and the file path. File names encode the chromosome,
# window start, and window size, e.g.
# "<chrom>.<start>.<size>_scoresPhastCons_250.wig_scores.bed".
# Index: chrom <tab> winstart <tab> winend <tab> filepath
> "$LOCAL_TMP/phast_index.tsv"
for f in "$PHAST_DIR"/*_scoresPhastCons_250.wig_scores.bed; do
    base=$(basename "$f")
    prefix="${base%_scoresPhastCons_250.wig_scores.bed}"
    chrom="${prefix%%.*}"
    rest="${prefix#*.}"
    wstart="${rest%%.*}"
    size="${rest#*.}"
...
echo "Indexed $(wc -l < "$LOCAL_TMP/phast_index.tsv") PhastCons window files."

# awk script: accumulates mean PhastCons score per feature via streaming,
# without ever sorting or fully loading the PhastCons window files.
#
# Approach: features are read first (NR == FNR) and indexed both by start
# and by end position. As phastCons score lines are then streamed in
# genomic order, a running sum/count of scores is maintained; each feature
# snapshots that running total the moment the stream reaches its start, and
# again the moment it reaches its end. The mean for a feature is simply the
# difference of those two snapshots divided by the difference in counts --
# this avoids ever storing per-base scores or re-scanning the phastCons
# stream per feature.
...
# processing the first streamed phastcons line. Insertion sort is fine:
...
END {
    # Edge case: no phastcons lines were streamed at all for this chromosome.
    if (!sorted) {
...
# Processes a single chromosome: extracts its CREs and PhastCons window
# files, then streams the window files through the awk script above to
# compute each CRE's mean PhastCons score. Run in parallel across
# chromosomes below.
run_chrom() {
    local CHR=$1
    local TMPDIR=$2
    ...
    if [[ ! -s "$TMPDIR/files_${CHR}.txt" ]]; then
        echo "WARN: No PhastCons window files for $CHR" >&2
        awk 'BEGIN{OFS="\t"} {print $0, "."}' "$TMPDIR/mus_${CHR}.bed" > "$TMPDIR/out_phastCons_${CHR}.bed"
        rm -f "$TMPDIR/mus_${CHR}.bed" "$TMPDIR/files_${CHR}.txt"
        return
    fi

    echo "Streaming $(wc -l < "$TMPDIR/files_${CHR}.txt") PhastCons window file(s) for $CHR..."
    ...
    xargs -a "$TMPDIR/files_${CHR}.txt" cat \
        ...
        > "$TMPDIR/out_phastCons_${CHR}.bed"

    rm -f "$TMPDIR/mus_${CHR}.bed" "$TMPDIR/files_${CHR}.txt"
}
export -f run_chrom

echo "[3/4] Streaming PhastCons mean per chromosome..."
...
find "$LOCAL_TMP" -maxdepth 1 -name "out_phastCons_*.bed" -print0 \
    | xargs -0 cat \
